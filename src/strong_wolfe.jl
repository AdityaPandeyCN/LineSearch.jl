...
# Dispatch on static types directly rather than using `stats === nothing` as a proxy.
...
# cubic interpolation with bisection fallback for degenerate discriminant / non-finite candidate
...
# default nonlinear-solve eval functor for static/GPU path (isbits)
...
# N&W Algorithm 3.6 (zoom)
# eval_fn(α) -> (ϕ, dϕ) is any callable satisfying this interface.
# (ϕ_hi, dϕ_hi) supplied by caller — avoids redundant evaluation at entry.
# Fixed iteration count with !done guard keeps this warp-uniform on GPU;
# do not rewrite into an early `break`.
...
# N&W Algorithm 3.5 (outer search)
# Fixed iteration count with !done guard keeps this warp-uniform on GPU.
@inline function _sw_search(eval_fn, ϕ_0, dϕ_0, c1, c2, α_init, α_max, maxiters, zoom_maxiters)
    ...
    for i in 1:maxiters
        if !done
            ...
            if (ϕ_i > ϕ_0 + c1 * α_i * dϕ_0) || (ϕ_i >= ϕ_prev && i > 1)
                # bracket is [α_prev, α_i]; pass both endpoints' (ϕ, dϕ) through
                α_z, ok_z = _sw_zoom(
            ...
            elseif dϕ_i >= zero(T)
                # bracket is [α_i, α_prev]; pass both endpoints' (ϕ, dϕ) through
                α_z, ok_z = _sw_zoom(
    ...
    if !done
        # last step actually tried, not α_prev (which may be stale/zero)
        α_out = α_i
        ok = false
    end
    return (α_out, ok)
