...
@inline function _sw_zoom(
        eval_fn, a_lo, a_hi, ϕ_0, dϕ_0, ϕ_lo, dϕ_lo, c1, c2, maxiters
    )
    T = typeof(a_lo)
    α_out = a_lo
    ok = false
...
# eval_fn(α) -> (ϕ, dϕ) is any callable satisfying this interface
@inline function _sw_search(eval_fn, ϕ_0, dϕ_0, c1, c2, α_init, maxiters)
    T = typeof(α_init)
    α_max = T(65536)
...
function CommonSolve.solve!(cache::StaticStrongWolfeLineSearchCache, u, du)
    ...
    α, ok = _sw_search(
        eval_fn, ϕ_0, dϕ_0, cache.c1, cache.c2, T(cache.α_init), cache.maxiters
    )
    ok && return LineSearchSolution(α, ReturnCode.Success)
